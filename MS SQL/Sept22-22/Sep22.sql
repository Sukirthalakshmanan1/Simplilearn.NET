...
Alter table book_c Add  cost int;
Alter table Member_c Add Member_name varchar(30);
Alter table book_c Add Author varchar(30);
...
create view display
as
select i.Lib_Issue_Id, b.Book_Name, m.Member_Name, b.Author , i.Issue_date from Book_C b inner join Issue_C i on b.Book_No = i.Book_No inner join Member_C m on i.Member_Id = m.Member_ID where (b.Cost between 500 and 750) and b.Category in ('Database' , 'RDBMS')

select * from display

--8)	Create a view with check option to display all the books that has price greater then 500.
create view view2 as
select * from Book_C where Cost > 500
with check option
...
insert into view2 values(100,'Stellar','Rob Stark', 850, 'Others')
select * from view2
insert into view2 values(16,'Black Man','Stanley', 180, 'Others')
-- can't insert 106
